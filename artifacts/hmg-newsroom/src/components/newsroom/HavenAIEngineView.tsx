...
const ACTIVE_MODULES = [
  { name: "Editorial Analysis Engine", status: "Local Brain Active", detail: "Headline variants, source checklist, gossip check, WP prep — all deterministic, memory-backed" },
...
const WORKS_NOW = [
  ...
  "Deterministic editorial suggestions",
...
export function HavenAIEngineView({
  ...
  const topRecommendedAction = useMemo(() => {
    ...
    if (!memoryItems.some((i) => i.type === "founder-voice"))
      return "Add a Founder Voice memory item — it powers Editorial Analysis, Social Factory, and Editorial Desk.";
    ...
    if (outputCount === 0)
      return "Memory loaded. Run Editorial Analysis or WebEdit to generate your first content output.";

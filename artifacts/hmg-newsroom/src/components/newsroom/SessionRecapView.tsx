  ...
  const nextActions = useMemo(() => {
    ...
    if (!memoryItems.some((i) => i.type === "founder-voice")) {
      actions.push({
        ...
        hint: "Editorial Desk, ARTBOT, and Social Factory all need voice context.",
        view: "founderkb",
        priority: "critical",
      });
    }
        ...
        {todayOutputs.length > 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-border/50 bg-card/30 px-4 py-4 text-center">
            ...
            <p className="text-[11px] text-muted-foreground/70 mt-1">
              Run ARTBOT Editorial, WebEdit, or Social Factory to start building outputs.
            </p>
          </div>
        )}
      </div>
